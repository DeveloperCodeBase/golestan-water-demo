...
سامانه با داده‌های بذر/ساختگی اجرا می‌شود و معماری آن در سطح نزدیک به تولید طراحی شده است. اتصال سرویس مدل زبانی برای چت‌بات **اختیاری** است.
...
- چت‌بات پاپ‌آپ تخصصی پروژه (دارای Scope Guard)
- نوسازی خودکار توکن در فرانت‌اند (برای جلوگیری از خطای انقضای نشست)
...
- نقشه GIS نمایشی منطقه سدها و گره‌های پایین‌دست (توپوگرافی/relief نمایشی)
- مدیریت سامانه:
  - کاربران و نقش‌ها
  - گزارش‌های ممیزی
  - تنظیمات سیستم
...
## انطباق با RFP (خلاصه)
- جمع‌آوری و کیفیت‌سنجی داده‌های هیدرولوژی/هواشناسی/مخزن/تقاضا
- پیش‌بینی سناریومحور (تر/نرمال/خشک) برای ورودی، تقاضا و وضعیت مخزن
- بهینه‌سازی چندهدفه با قیود ایمنی، محیط‌زیست و اولویت‌بندی بخش‌ها
- مقایسه تصمیم پیشنهادی با روش سنتی (Baseline Mock)
- پشتیبانی بحران سیلاب/خشکسالی و مرکز هشدار
- گزارش مدیریتی/فنی و صفحه آموزش بهره‌برداران
...
- برای فعال‌سازی چت‌بات هوشمند، مقدار `OPENROUTER_API_KEY` را در `apps/api/.env` قرار دهید (نام متغیر فنی ثابت است).
- برای اجرای Docker، مقادیر DB/Redis از `docker-compose.yml` کنترل می‌شوند و به‌صورت خودکار همگام هستند.

---

## اجرای سریع با Docker

### 1) تنظیم متغیرهای Compose (اختیاری اما توصیه‌شده)
```bash
cp .env.example .env
```
...
اگر قبلا پروژه را با تنظیمات DB متفاوت بالا آورده‌اید، قبل از اجرای مجدد این دستور را بزنید:
```bash
...
### 3.1) دسترسی از بیرون سرور با IP
...
- به‌صورت پیش‌فرض پورت‌های `5432` و `6379` فقط روی `localhost` bind می‌شوند و از بیرون سرور در دسترس نیستند.

### 3.2) تست سریع بعد از بالا آمدن سرویس‌ها
```bash
curl http://localhost:8000/health
curl http://localhost:3000/backend/health
```
هر دو باید وضعیت `ok` برگردانند.
...
## تنظیم سرویس چت هوشمند (اختیاری)

برای فعال‌سازی پاسخ‌دهی مدل زبانی در چت‌بات، مقادیر زیر را در `apps/api/.env` تنظیم کنید:
...
- اگر تنظیمات سرویس مدل زبانی ناقص باشد، پاسخ fallback قواعدمحور برمی‌گرداند.
...
- اگر خطای `token_expired` یا `invalid_token` دریافت کردید:
  - رفتار استاندارد سامانه اکنون `401` است (نه `500`).
  - یک‌بار Logout/Login انجام دهید یا `localStorage` مرورگر را پاک کنید.

- اگر ابتدای بالا آمدن سامانه، `GET /backend/health` موقتاً خطا داد:
  - چند ثانیه پس از Boot دوباره تست کنید؛ تا آماده شدن API، پروکسی Next ممکن است `ECONNREFUSED` بدهد.

- اگر قبلاً خطای `spawn /bin/sh EACCES` در وب دیده بودید:
  - CMD کانتینر وب اصلاح شده و `next` مستقیم با Node اجرا می‌شود (بدون `npm run start`).
  - با `docker compose up -d --build` ایمیج جدید را جایگزین کنید.

- اگر در لاگین‌های پشت‌سرهم سریع با خطای تداخل refresh token مواجه شدید:
  - مشکل با افزودن `jti` یکتا به JWT رفع شده و لاگین تکراری پایدار است.

- اگر خطای `password authentication failed for user "postgres"` گرفتید:
  - اسکریپت رفع سریع را اجرا کنید:
  - `./scripts/fix_db_auth.sh`
  - سپس وضعیت سرویس‌ها را چک کنید:
  - `docker compose ps`
  - اگر مشکل ادامه داشت:
  - `docker compose down -v && docker compose up --build`

- اگر چت‌بات پیام «سرویس پاسخ هوشمند در دسترس نیست» داد:
  - مطمئن شوید کلید در `apps/api/.env` تنظیم شده است.
  - مقدار کلید باید بدون فاصله اضافه و بدون کوتیشن باشد.
...
## نکات مهم توسعه
- مسیر چت‌بات هوشمند اختیاری است و در نبود کلید، fallback داخلی فعال می‌ماند.
